...
cvar_t *touch_pitch;
cvar_t *touch_yaw;
cvar_t *touch_forwardzone;
cvar_t *touch_sidezone;
cvar_t *touch_nonlinear_look;
cvar_t *touch_pow_mult;
cvar_t *touch_pow_factor;
cvar_t *touch_exp_mult;
cvar_t *touch_grid_enable;
cvar_t *touch_grid_count;
cvar_t *touch_config_file;
cvar_t *touch_enable;
cvar_t *touch_in_menu;
cvar_t *touch_joy_radius;
cvar_t *touch_dpad_radius;
cvar_t *touch_move_indicator;
cvar_t *touch_highlight_r;
cvar_t *touch_highlight_g;
cvar_t *touch_highlight_b;
cvar_t *touch_highlight_a;
cvar_t *touch_precise_amount;
cvar_t *touch_joy_texture;

// code looks smaller with it
#define B(x) button->x
#define SCR_W (vid_conwidth.value)
#define SCR_H (vid_conheight.value)
...
void Touch_WriteConfig( void )
{
	...
	if( f )
	{
		...
		FS_Printf( f, "//=======================================================================\n");
		FS_Printf( f, "//\tCopyright SDLash3D team & XashXT group ©\n" );
	...
	for( i = 0; i < g_LastDefaultButton; i++ )
	{
		...
		if( g_DefaultButtons[i].aspect && g_DefaultButtons[i].round == round_aspect )
		{
			if( g_DefaultButtons[i].texturefile[0] == '#' )
				y2 = y1 + g_DefaultButtons[i].aspect + touch.swidth*2/SCR_H;
			else
...
void Touch_DeleteProfile_f( void )
{
	...
	// delete profile
//	FS_Delete( va(vabuf,256, "touch_profiles/%s.cfg", Cmd_Argv( 1 )));
}
...
void Touch_Init( void )
{
	...
	Touch_AddDefaultButton( "loadquick", "touch_default/load.tga", "load quick", 0.760000, 0.000000, 0.840000, 0.151486, color, 2, 1, 16 );
	Touch_AddDefaultButton( "savequick", "touch_default/save.tga", "save quick", 0.840000, 0.000000, 0.920000, 0.151486, color, 2, 1, 16 );
...
//	Touch_AddDefaultButton( "tduck", "touch_default/tduck.tga", ";+duck", 0.560000, 0.833171, 0.620000, 0.946785, color, 2, 1, 0 );
	...
	Touch_AddDefaultButton( "menu", "touch_default/menu.tga", "togglemenu", 0.000000, 0.833171, 0.080000, 0.984656, color, 2, 1, 0 );
	...
	touch_config_file = Cvar_Get( "touch_config_file", "touch.cfg", CVAR_SAVE, "current touch profile file" );
	...
	// input devices cvar
	touch_enable = Cvar_Get( "touch_enable", "1", CVAR_SAVE, "enable touch controls" );
...
void Touch_InitConfig( void )
{
	...
	if( FS_FileExists( touch_config_file->string ) )
		Cbuf_AddText( va(vabuf,256, "exec \"%s\"\n", touch_config_file->string ) );
...
void Touch_DrawButtons( touchbuttonlist_t *list )
{
	...
	for( button = list->first; button; button = button->next )
	{
		if( Touch_IsVisible( button ) )
		{
			...
			if( B( fadespeed ) )
			{
				button->fade += B( fadespeed ) * cl.realframetime;
...
static void Touch_Motion( touchEventType type, int fingerID, float x, float y, float dx, float dy )
{
	...
	if( fingerID == touch.look_finger )
	{
		...
		if( VEC_IS_NAN( dx ) || VEC_IS_NAN( dy ) )
			return;
...
int IN_TouchEvent( touchEventType type, int fingerID, float x, float y, float dx, float dy )
{
	...
	{
		x = x / vid.mode.width;
		dx = dx / vid.mode.width;
		y = y / vid.mode.height;
		dy = dy / vid.mode.height;
	}

#if 0
	// simulate menu mouse click
	if( key_dest != key_game && !touch_in_menu->integer )
	{
		touch.move_finger = touch.resize_finger = touch.look_finger = -1;
...
{
	cl.cmd.forwardmove += touch.forward * cl_forwardspeed.value;
	cl.cmd.sidemove += touch.side * cl_sidespeed.value;
	cl.viewangles[0] += touch.pitch * cl_pitchspeed.value * cl.realframetime;
	cl.viewangles[1] += touch.yaw * cl_yawspeed.value * cl.realframetime;
